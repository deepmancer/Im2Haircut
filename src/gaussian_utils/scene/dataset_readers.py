...
from utils_3dgs.sh_utils import SH2RGB
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    if not os.path.exists(ply_path):
        ...
        num_pts = 100_000
        print(f"Generating random point cloud ({num_pts})... in [-1.3, 1.3]")
...
def readSyntheticCameras(extrinsics_all, intrinsics_all, images_folder, resolution=(1024, 1024)):
    cam_infos = []
    ...
    for i in range(extrinsics_all.shape[0]):
        ...
        cx = intrinsics_all[i][0][2].cpu()
        cy = intrinsics_all[i][1][2].cpu()
        ...
        cam_infos.append(CameraInfo(uid=0, R=R, T=T, FovY=FoVy, FovX=FoVx, cx=cx, cy=cy, image=image,
                                    image_path=image_path, image_name=image_name, width=resolution[0], height=resolution[1]))
...
def readSyntheticSceneInfo(path, images, eval, llffhold=2):
    ...
    print('inside synth scene info')
    try:
        print('load projection npy')
        camera_dict = np.load(f'{path}/projection.npy')
    except:
        print('load cameras npz')
    ...
    for world_mat in camera_dict:
        ...
        intrinsics_all.append(scale_matrix(torch.from_numpy(intrinsics).float(), 1)) #scale matrix 2
    ...
    img0 = os.listdir(os.path.join(path, reading_dir))[0]
    width, height = Image.open(os.path.join(path, reading_dir, img0)).size

    cam_infos_unsorted = readSyntheticCameras(extrinsics_all=pose_all_inv, intrinsics_all=intrinsics_all, images_folder=os.path.join(path, reading_dir), resolution=(width, height))
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)
