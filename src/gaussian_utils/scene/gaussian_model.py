...
from src.utils.general_utils import strip_symmetric, build_rotation
import math
...
class GaussianModel:
    ...
    def get_covariance(self, scaling_modifier = 1, return_full_covariance = False):
        ...
        S = torch.zeros((s.shape[0], 3, 3), dtype=torch.float, device=self.device)
    ...
    def get_direction_2d(self, viewpoint_camera):
        ...
        i = torch.arange(self.scaling.shape[0], device=self.device)[:, None].repeat(1, 3).view(-1)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        self._label = nn.Parameter(inverse_sigmoid(0.5 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device=self.device)).requires_grad_(True))
    ...
    def load_ply(self, path):
        ...
        self._orient_conf = nn.Parameter(torch.tensor(orients_conf, dtype=torch.float, device=self.device).requires_grad_(True))
        self._label = nn.Parameter(torch.tensor(labels, dtype=torch.float, device=self.device).requires_grad_(True))
        if len(scale_names): self._scaling = nn.Parameter(torch.tensor(scales, dtype=torch.float, device=self.device).requires_grad_(True))
        if len(rot_names): self._rotation = nn.Parameter(torch.tensor(rots, dtype=torch.float, device=self.device).requires_grad_(True))
        if len(cov_names): self._covariance = nn.Parameter(torch.tensor(covs, dtype=torch.float, device=self.device).requires_grad_(True))
