...
from src.utils.camera_opt_utils import lie
from src.utils.graphics_utils import getWorld2View2, getProjectionMatrix, fov2focal

from src.gaussian_utils.graphics_utils import getProjectionMatrixShift
...
class CameraMini(nn.Module):
    def __init__(self, R, T, FoVx, FoVy,
                 width, height, cx, cy, trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device = "cuda", white_background = False,
                 ):
        super(CameraMini, self).__init__()
...
#         ).transpose(0,1).cuda()
        ...
        self._projection_matrix = getProjectionMatrixShift(
            znear=self.znear, 
            zfar=self.zfar, 
            fovX=self._FoVx, 
            fovY=self._FoVy,
