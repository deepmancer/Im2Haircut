...
python scripts/img2masks.py --root_real_imgs /localhome/aha220/HairProjects/Im2Haircut/data/new_data --checkpoint_sam $SAM_PATH
python scripts/img2strand.py --root_real_imgs /localhome/aha220/HairProjects/Im2Haircut/data/new_data

echo "Finished direction maps and silhouette estimation."

# Step 2: Orientation maps and confidence maps
cd $PROJECT_DIR && python ./preprocess_dataset/calc_gabor_mask.py --img_path "$path/$path_set/resized_img" \
...
depth-pro-run -i /localhome/aha220/HairProjects/Im2Haircut/data/new_data/resized_img -o /localhome/aha220/HairProjects/Im2Haircut/data/new_data/depth_apple_pro

echo "Finished depth estimation."

# Step 4: data alignment
cd $PROJECT_DIR && python ./preprocess_dataset/calc_alignment.py --img_path "$path/$path_set/resized_img" \
    --hair_path "$path/$path_set/seg" \
    --all_paths_for_processing seg body_img orientation_maps depth_apple_pro strand_map \
    --gt_img_path "/localhome/aha220/HairProjects/Im2Haircut/data/aligned_image.png"
...
conda deactivate && conda activate clip
cd $PROJECT_DIR &&  python ./preprocess_dataset/calc_proj_matx.py --root_path "$path/$path_set"
cd $PROJECT_DIR &&  python ./preprocess_dataset/calc_proj_matx.py --root_path "$path/$path_set"  --save_postfix "_aligned"
